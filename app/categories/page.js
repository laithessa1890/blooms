...
export default function CategoriesGrid() {
  return (
    <section className="px-4 py-10 text-right" dir="rtl">
      <h2 className="text-2xl sm:text-3xl font-bold text-[#C05370] mb-6">تصنيفات الكتب</h2>
        ...
        {categories.map((cat, idx) => (
          <Link key={idx} href={cat.link}>
            <div className="border border-gray-300 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition">
              ...
              <div className="p-3 text-sm sm:text-base font-semibold text-gray-800">{cat.title} →</div>
            </div>
          </Link>
        ))}
      </div>
